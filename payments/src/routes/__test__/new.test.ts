...
it('return a 404 when purchasing an order that does not belong to user',async()=>{
...
it('returns a  400 when purchasing a cencelled order',async()=>{
...
it('payment save successfully ',async()=>{
    const userId=new mongoose.Types.ObjectId().toHexString()
    const price = Math.floor(Math.random() *100000)
